...
import { Flex, Grid, GridItem, HStack, Input, InputGroup, InputLeftElement, Text, Tooltip, VStack } from '@chakra-ui/react';
...
export interface BookingCategoryConProps {
    ...
    penal: (mdt_charges_categories & mdtCharges)[] | undefined
    characterError: any;
    penalError: any;
    addChagre: (a: mdt_charges) => void;
}

const BookingCategoryCon: React.SFC<BookingCategoryConProps> = ({ addChagre, character, penal, characterError, penalError }) => {
    ...
    return (
        <Flex minW="70%" maxW="70%" w="70%" h="full" flexDir="column">
            <InputGroup variant="filled" mb="3">
                ...
                <Input placeholder="Filter Charges" value={filter} onChange={(evt) => setFilter(evt.target.value)} />
            </InputGroup>
                            ...
                            {penal.map(cat => {
                                return (
                                    <Flex w="full" key={cat.categoryid} flexDir="column">
                                        <Flex mb="1" background="gray.700" py="1.5" justifyContent="center" alignItems="center" borderTopRadius="md" w="full">{cat.name}</Flex>
                                        <Grid alignItems="center" justifyItems="center" templateColumns="repeat(4, 1fr)" gap={1}>
                                            {cat.mdt_charges.filter(_c => _c.name?.toLocaleLowerCase().match(filter)).map(char => <ChargeBox addChagre={addChagre} charge={char} key={char.chargeid} />)}
                                        </Grid>
                                    </Flex>
                                )
                            })}
