...
const ChargeBox: React.SFC<ChargeBoxProps> = ({ charge, addCharge }) => {
  return (<GridItem borderRadius="sm" w="12rem" h="5rem" background="blue.100">
    {charge.name}
  </GridItem>);
}
...
const Home: React.SFC<HomeProps> = ({ session }) => {
  ...
  const { data: character, error: characterError } = useSWR(`/api/citizen/?citizenid=${cuid}`) as SWRResponse<BookingSWR, any>;
    ...
    <Layout>
      <Flex flexDir="column" w="full" h="full">
        ...
        <Flex w="full" flexGrow={1}>
          <BookingCategoryCon addChagre={addCharge} character={character} penal={penal} characterError={characterError} penalError={penalError} />
          <Flex flexGrow={1} h="full" background="gray">
            {JSON.stringify(selectedCharges)}
          </Flex>
        </Flex>
      </Flex>
    </Layout >
